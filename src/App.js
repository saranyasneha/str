...
function App() {
  ...
  const deleteHandler = (id,name) => {
    if(window.confirm(`do you want to delete ${name} details??`)){
    axios.delete(`http://localhost:1337/api/employee/${id}`).then(del => {
      getEmployeeData()
    }
    )
    console.log(id);
              ...
              <tr key={item.id}>
                ...
                <td className='action'>
                  <div className='buttons'>
                    ...
                    <i class="fa fa-trash delete" onClick={() => deleteHandler(item.id,item.attributes.EmployeeName)}></i>
                  </div>
                </td>
              </tr>
            )
